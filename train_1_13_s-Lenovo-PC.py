...
def load_txt_pre():
    pre_data=open('E:/tianyidata/pre_1_17_3.txt','r')
    keyline={}
...
def train_load():
    time1=time.time()
    train=open('E:/tianyidata/feature_data_line70.pkl','rb')
...
def load_fm():
    ...
    w_data=open('E:/tianyidata/pre_1_17_3_r3.txt','w')
    NN=0
    mypre=load_txt_pre()
    #mypre=train_main()
    #print(mypre)
    ...
    for oone in mypre.keys():
        ...
        if oneSimilarity>0.005:
            NN=NN+1
            print(NN)
            spre=str(pre_line)
            spre=spre[1:-1]
            spre=re.sub(' ','',spre)
            w_data.write(str(kuser)+'\t'+spre+'\n')
        #else:
