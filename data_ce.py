...
def load_train_data():
    train_data=open('E:/tianyidata/01_part-r-00000_2_.txt','r')
...
#load_train_data()
def train():
    w_data=open('E:/tianyidata/pre_1_10_c.txt','w')
    ...
    for one in feature.keys():
        ...
        for eday in range(1,8):
            for v in range(1,11):
                ...
                precount=(avecount*(chaday/aveday)+(chatype/avetype)*avecount)*0.5
                prefline.append(int(precount))
        spre=str(prefline)
        if prenum>=5:
            w_data.write(str(one)+'\t'+str(spre[1:-1])+'\n')
